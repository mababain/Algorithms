...
function alg(data) {
  let [benchInfo, blocksCoords] = data.toString().trim().split('\n')
  blocksCoords = blocksCoords.split(' ').map((n) => +n)
  const [benchLength, _] = benchInfo.split(' ').map((n) => +n)
  if (
    benchLength % 2 !== 0 &&
    blocksCoords.indexOf(Math.floor(benchLength / 2)) !== -1
  ) {
    return blocksCoords[blocksCoords.indexOf(Math.floor(benchLength / 2))]
  } else {
    const center = benchLength / 2
    let left = blocksCoords[0]
    let right = blocksCoords[blocksCoords.length - 1]
    for (let i = 0; blocksCoords[i] < center; i++) {
      if (center - left > center - blocksCoords[i]) left = blocksCoords[i]
    }
    for (let i = blocksCoords.length - 1; blocksCoords[i] >= center; i--) {
      if (center - right < center - blocksCoords[i]) right = [blocksCoords[i]]
    }
    return `${left} ${right}`
  }
}
